...
<head>
    ...
    <style>
        ...
        input[type="text"], input[type="number"], select { width: 100%; padding: 10px 12px; border: none; border-radius: 8px; background: #2a2a2a; color: #e0e0e0; font-size: 16px; }
    ...
    <div class="container">
        ...
        <form action="{{ route('livros.update', $livro) }}" method="POST">
            ...
            <label>Ano de Publicação:
                <input type="number" name="ano_publicacao" value="{{ old('ano_publicacao', $livro->ano_publicacao) }}" min="1000" max="{{ date('Y') }}">
            </label>
            <label>Status:
                <select name="status_livro">
                    <option value="disponivel" {{ old('status_livro', $livro->status_livro) == 'disponivel' ? 'selected' : '' }}>Disponível</option>
                    <option value="emprestado" {{ old('status_livro', $livro->status_livro) == 'emprestado' ? 'selected' : '' }}>Emprestado</option>
                </select>
            </label>
